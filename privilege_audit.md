...
All five lab stages were completed. Findings and proposed corrective actions are documented below; remediation was outside the completed exercise.
...
The instructor supplied the initial CSV and name-resolved JSON because my account's directory visibility was limited. I distinguish those supplied exports from my captured CLI execution and portal observations. Findings describe the access class, while identities and lab answers remain private.
...
| Azure CLI | Scriptable enumeration with principal IDs, roles, scopes, and unresolved-name fields | My lab command targeted one resource group. Broader scope is possible with CLI options and subscription iteration. Empty names can result from failed directory lookups as well as deletion; ordinary assignment listing does not inventory all PIM eligibility. | Compared live output with the supplied name-resolved export before classifying the orphan. |
...
Severity is an analyst assessment of the lab conditions, not a CVSS score. High means broad management privilege requiring priority review; Medium means a narrower stale entitlement and lifecycle-control gap. Business need and production impact would be validated with the resource owner before changes.
...
I began with the instructor-provided IAM CSV. One user appeared repeatedly with Owner assignments, including a subscription-level grant identified by the lab and further grants across resource scopes.
...
![Redacted IAM export with repeated Owner rows labeled USER-A](assets/privilege-audit/01-iam-baseline.png)
...
The relevant JSON record retained a principal ID and resource-group scope. Its `principalName` was empty, its `principalType` was `User`, and its assigned role was **Reader**. The lab explanation identified it as an account that had been deleted without removing the assignment.
...
The lab did not demonstrate surviving tokens or exploitation. Nor does the evidence establish that an attacker could create a new account with an arbitrarily chosen object ID. Reusing a display name does not establish identity continuity. If an original identity is restored, its associated access should be reviewed as part of restoration rather than assumed safe.

![Sanitized JSON preserving the empty principalName, User type, and Reader role](assets/privilege-audit/02-orphaned-reader.png)
...
Resource Graph can query across multiple accessible subscriptions. It does not bypass RBAC, and a directory-level selection does not grant tenant-wide visibility. I treated the results as evidence about the scopes the account could inspect, rather than proof that inaccessible areas were clear. [Reference](https://learn.microsoft.com/en-us/azure/governance/resource-graph/overview)

![Resource Graph inventory query with 83 results and identifiers redacted](assets/privilege-audit/03-resource-graph.png)
...
![PIM Groups view showing eligible Member assignment with the group name redacted](assets/privilege-audit/04-pim-eligibility.png)

*Figure 4. Eligible group membership in PIM. Permanent here describes the eligibility shown in this view; it does not establish permanently active resource privilege. The group name is removed.*
...
![Resource-group Check access view showing active time-bound Operative and eligible assignment states](assets/privilege-audit/05-effective-access.png)
...
![Redacted Owner export excerpt preserving condition version and role-assignment condition expression](assets/privilege-audit/06-owner-evidence.png)
...
- The PIM group screenshot establishes eligibility; the resource-group view establishes time-bound Operative access. Neither is evidence that I activated Owner.
